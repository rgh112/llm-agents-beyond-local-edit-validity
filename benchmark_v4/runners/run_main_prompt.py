#!/usr/bin/env python3
"""Package A: Main prompt experiment.

3 envs × 6 prompts × 50 seeds × M0 = 900 episodes.
Core result table for the paper.
"""
...
# GB1 fitness_target is set after calibration; initial runs use the env default.
# To override: set GB1_FITNESS_TARGET env var
GB1_FITNESS_TARGET = float(os.environ.get("GB1_FITNESS_TARGET", "5.0"))

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_DIR = f"results_v4/main_prompt_{timestamp}"
...
print()
print("=" * 90)
print(f"MAIN PROMPT EXPERIMENT COMPLETE: {combo_count} episodes in {sweep_time:.0f}s")
print("=" * 90)

for env_name in envs:
    print(f"\n{'='*25} {env_name.upper()} {'='*25}")
    env_results = [r for r in results_table if r["env"] == env_name]
    print(f"{'Prompt':<22} {'Strat':>5} {'Task':>5} {'SR':>8} {'Steps':>6} "
          f"{'Surf':>5} {'Struct':>6} {'Fin':>5} {'1stF':>5}  Top Failures")
    print("-" * 110)
    for r in env_results:
        ...
        print(f"{r['prompt']:<22} "
              ...
              f"{r['avg_steps']:>5.1f} "
              f"{r['surface_failure_count']:>5} {r['structural_failure_count']:>6} "
...
for env_name in envs:
    ...
    sc = env_r.get("scaffold", {}).get("SR", 0)
    ...
    print(f"  Scaffold:           scaffold({sc:.0%}) - zero_shot({zs:.0%}) = {sc-zs:+.0%}")
    print(f"  Generic self-check: self_check_generic({sg:.0%}) - zero_shot({zs:.0%}) = {sg-zs:+.0%}")
    print(f"  Task-specific:      self_check({st:.0%}) - self_check_generic({sg:.0%}) = {st-sg:+.0%}")

# Save
summary_path = Path(OUTPUT_DIR) / "main_prompt_summary.json"
